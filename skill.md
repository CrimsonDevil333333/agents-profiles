...
> **127 agents. 20 categories. One skill to rule them all.**
...
## 127 Agents Exist — SELECT, Don't Create

**Never generate new `.md` files.** The 127 profiles cover every common role. Only create a new profile if the role genuinely doesn't exist (rare).
...
**Final count must be 6-15 agents.** If Tier 1 + Tier 2 + Tier 3 exceeds 15, prioritize Tier 2 over Tier 3. Never select all 127.
...
Only copy the agents selected in the roster (from Step 4). Do NOT copy all 127 unless the project genuinely needs every role.
...
> Agents selected from 127 pre-built profiles at
...
| **Never load all 127** | Only load the agent(s) needed for current task |
...
| Creating new agents | 127 already cover it | Select from existing |
...
1. **SELECTION is primary** — Default action is to select from 127 existing profiles. Generation is a fallback.
...
*"The 127 agents are already built. Your job is not to create — it's to select, load, and delegate. Be the conductor, not the musician."*
